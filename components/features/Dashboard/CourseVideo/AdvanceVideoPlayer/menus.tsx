...
export function Settings({ placement, tooltipPlacement, iconProps = {} }: { placement: string, tooltipPlacement: string, iconProps: any }) {
  return (
    <Menu.Root className="parent z-50">
      ...
      <Menu.Content className={menuClass} placement={placement as any}>
        <AutoPlay />
        <AutoNext />
        <AutoSkip />
        <CaptionSubmenu />
  ...
  return (
    <Menu.Root>
      <SubmenuButton
        ...
        hint={settings?.autoplay !== undefined
          ? options.find((option) => option.value === settings?.autoplay)
            ?.label as string
          : options.find((option) => option.selected)?.label as string}
        icon={() => (
...
function SubmenuButton({ label, hint, icon: Icon, disabled }: { label: string, hint: string, icon: any, disabled: boolean }) {
  return (
