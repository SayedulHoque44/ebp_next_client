  ...
  return (
    <div className="py-10 relative">
      {!isLoading && user?.role !== "Admin" && (
        <DayPrivateRoute loggedUser={user} dayCount={[0, 2, 4, 6]} />
      )}
      ...
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          ...
          {Contents?.data?.result?.length &&
            Contents?.data?.result?.length > 0 &&
            Contents.data.result.map(
              (item: ICourseVideoContentType, index: number) => (
                <motion.div
                  key={item._id}
