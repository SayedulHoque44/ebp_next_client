...
import { useGetSystemsQuery } from "../../../../redux/Api/SystemManagmentApi";
    ...
    <div className="space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        {isLoading ? (
          ...
          <System isLoading={isLoading} AllSystems={AllSystems} />
        ) : (
...
const System = ({ AllSystems, isLoading }) => {
  const [selectedTab, setSelectedTab] = useState("");

  useEffect(() => {
    if (AllSystems?.length > 0) {
      setSelectedTab(AllSystems[0].category);
    }
  }, [AllSystems]);
  ...
  if (!isLoading && AllSystems.length > 0) {
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-8">
            ...
            <div className="p-3 flex flex-col gap-2">
              {AllSystems.map((system, index) => {
                const isSelected = selectedTab === system.category;
                return (
                  <button
                    key={index}
        ...
        <div className="w-full lg:w-3/4 min-w-0">
          {selectedTab ? (
            <div className="animate-fadeIn">
              <SingleSystem
                systemId={
                  AllSystems.find((ele) => ele.category === selectedTab)._id
                }
              />
            </div>
          ) : (
    ...
    return <Warning title={"System Tabs Found"} info={"Try Again Later"} />;
  }
};
