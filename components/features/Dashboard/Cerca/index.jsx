...
import { localBaseUrl } from "../../../redux/api/baseApi";
...
const fetchQuizSearchResults = async ({ queryKey }) => {
  ...
  const [res, vero, falso] = await Promise.all([
    axios.get(
      `${localBaseUrl}Quizzes?page=${page}&limit=10&sort=createdAt&searchTerm=${encodedSearch}`
    ),
    axios.get(
      `${localBaseUrl}Quizzes?page=1&limit=1&sort=createdAt&searchTerm=${encodedSearch}&answer=V`
    ),
    axios.get(
      `${localBaseUrl}Quizzes?page=1&limit=1&sort=createdAt&searchTerm=${encodedSearch}&answer=F`
    ),
  ]);
...
const Cerca = () => {
  ...
  const queryKey = useMemo(
    () => ["quiz-search", searchText.trim().toLowerCase(), page],
    [searchText, page]
  );
  ...
  const handleChange = useCallback(
    (e) => {
      ...
      if (!value || value.trim().length === 0) {
        ...
        queryClient.removeQueries({ queryKey: ["quiz-search"] });
        return;
      }
